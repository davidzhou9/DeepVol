    ...
    def build(self):
        ...
        with tf.variable_scope('forward'):
            ...
            for t in range(0, self._num_time_interval-1):
                ...
                ### changes being made
                curr_Factors = self._x[:, :, t] # get x (y, z if exists) at first time step
                curr_Stock_Price = self._x[:, 0, t]
                print("curr_Factors: ", curr_Factors)
                print("curr_Stock_Price: ", curr_Stock_Price)
                print("current_Sample[0, :]: ", output_z[t])
           
                sigma_SigmaT = self.calculate_Diffusion_Mat(curr_Factors)
                mat_Sqrt = sy.linalg.sqrtm(sigma_SigmaT)
                grads = np.linalg.solve(np.transpose(mat_Sqrt), output_z[t])
                print("Sigma_Sigma^T: ", sigma_SigmaT)
                print("mat_Sqrt: ", mat_Sqrt)
                print("grads: ", grads)
                
                delta_Loss_Terms = delta_Loss_Terms + tf.reduce_sum(tf.math.abs(grads - self._bsde.deltaCalc(self._x[:, :, t], t)))
                ###
            ...
            # added for delta tuning
            self._loss = self._loss + delta_Loss_Terms
