...
class PricingOptionNormal(Equation):
    ...
    def deltaCalc(self, stockPrices, t_Index):
        tau = self.total_time - t_Index * self._delta_t # time to maturity
        d1 = (tf.math.log(stockPrices / self._strike) + (self._r - self._sigma**2 / 2) * tau) / (self._sigma * tau)
        d2 = d1 - self._sigma * math.sqrt(tau)
        
        dist = tfd.Normal(loc = 0, scale = 1)
        
        deltas = tf.math.multiply(stockPrices, dist.cdf(d1)) - math.exp(-self._r * tau) * self._strike * dist.cdf(d2)
        return deltas
